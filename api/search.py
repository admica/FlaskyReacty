...
@search_bp.route('/api/v1/test/search', methods=['GET'])
@jwt_required()
@rate_limit()
def test_search():
    """Test endpoint to verify IP search functionality using test data"""
    try:
        # Ensure test tables exist with data
        if not ensure_test_tables_exist():
            return jsonify({
        ...
        test_cases = [
            {
                ...
                'data': {
                    ...
                    'expected_sensor': 'test_sensor1',
                    'expected_device': 'napa0'
                }
            },
            {
                ...
                'data': {
                    'dst_ip': '10.1.0.50',
                    'expected_subnet': '10.1.0.0/24',
                    'expected_sensor': 'test_sensor1',
                    'expected_device': 'napa0'
                }
            },
            {
                'name': 'Source-Destination pair',
                'data': {
                    'src_ip': '172.16.0.25',
                    'dst_ip': '192.168.2.50',
                    'expected_src_subnet': '172.16.0.0/24',
                    'expected_dst_subnet': '192.168.2.0/24',
                    'expected_sensor': 'test_sensor2',
                    'expected_device': 'napa0'
                }
            },
            {
                'name': 'Different device pair',
                'data': {
                    'src_ip': '10.0.0.25',
                    'dst_ip': '172.16.1.50',
                    'expected_src_subnet': '10.0.0.0/24',
                    'expected_dst_subnet': '172.16.1.0/24',
                    'expected_sensor': 'test_sensor3',
                    'expected_device': 'napa1'
                }
            }
        ]
        ...
        for test in test_cases:
            ...
            request_data = {k: v for k, v in test['data'].items() if k in ['src_ip', 'dst_ip']}

            # Make request to search endpoint
            with search_bp.test_request_context(json=request_data):
                response = search_ip()
                ...
                # Verify matches based on test case
                if 'dst_ip' in request_data and 'src_ip' in request_data:
                    # Source-destination pair test
                    test_passed = any(
                        str(match['src_subnet']) == test['data']['expected_src_subnet'] and
                        str(match['dst_subnet']) == test['data']['expected_dst_subnet'] and
                        match['sensor'] == test['data']['expected_sensor'] and
                        match['device'] == test['data']['expected_device']
                        for match in matches
                    )
                else:
                    # Single IP test
                    test_passed = any(
                        str(match['subnet']) == test['data']['expected_subnet'] and
                        match['sensor'] == test['data']['expected_sensor'] and
                        match['device'] == test['data']['expected_device']
                        for match in matches
                    )
